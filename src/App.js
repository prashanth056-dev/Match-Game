...
class App extends Component {
  ...
  imageClick = imgUrl => {
    ...
    if (image === imgUrl) {
      ...
      this.setState(prev => ({score: prev.score + 1, image: newImg}))
    } else {
      this.setState({isTrue: true})
      clearInterval(this.timerId)
    }
  ...
  render() {
    const {isTrue, category, score, timer, image} = this.state
    const filteredLst = imagesList.filter(
    ...
    return (
      <div className="MainBg">
        ...
        <div className="bottom-cont">
          {!isTrue && (
            <div className="card">
              <img src={image} alt="match" className="mainImg" />
              <ul className="tab-names">
                {tabsList.map(eachItem => (
                  <li key={eachItem.tabId}>
                    <button
                      className={`btn ${
                        eachItem.tabId === category ? 'active' : ''
                      }`}
                      type="button"
                      onClick={() => this.tabChange(eachItem.tabId)}
                    >
              ...
              <ul className="thumb-img">
                {filteredLst.map(eachItem => (
                  <li key={eachItem.id} className="thumb-list">
                    <button
                      type="button"
                      className="btn"
                      onClick={() => this.imageClick(eachItem.imageUrl)}
                    >
                      <img
          ...
          {isTrue && (
            <div className="card2">
              ...
              <p>Your Score</p>
              <h1>{score}</h1>
              <button type="button" className="playAgain" onClick={this.reset}>
                <img
